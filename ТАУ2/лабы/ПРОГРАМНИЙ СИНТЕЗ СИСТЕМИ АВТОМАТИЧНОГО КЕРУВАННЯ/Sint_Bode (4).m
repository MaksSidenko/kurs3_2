...
tr=1.6;       % бажаний час регулювання
kob=128;     % загальний коеф. передачі об'єкта керування (ОК)
kv=192;      % kv бажаний коеф. передачі розімкнутого контуру САК
...
zapsv=16;   % ордината лівого кінця середнточастотної асимптоти
              % бажаної ЛАЧХ, дБ
zapsn=16;
% wzr=round(33.78);    % запас стійкості, дБ       % завдана частота зрізу 
wzr=29;
kbb=1;  % Вибір бажаної ЛАЧХ: 0 - спрощена, 1- класична
...
nahn=40; % нахил низькочастотної з'єднувальної асимптоти bc. Її рівняння
         % Ln-nahn*log10(w)
nahv=40; % нахил високочастотної з'єднувальної асимптоти aan. Її рівняння
         % Lv-nahv*log10(w)
